...
function App() {
  ...
  const [deepgramStatus, setDeepgramStatus] = useState<DeepgramStatus | null>(
    null,
  );
  ...
  const testMicrophone = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        if (maxLevel > 10) {
          console.log(
            "✅ SUCCESS! Microphone is working. Max level:",
            maxLevel,
          );
  ...
  const startManualRecording = async () => {
    try {
      ...
      if (audioTrack.readyState !== "live") {
        throw new Error(
          " Microphone track is not live. Please check your microphone connection.",
        );
      }
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          if (maxAudioLevel === 0) {
            console.warn(
              "⚠️ Warning: Audio level was 0 during recording. Microphone may be muted in Windows.",
            );
            console.warn(
              "⚠️ If transcription is incorrect, check Windows Sound Settings > Input volume",
            );
          }
          ...
          const transcriptResult =
            await electronAPI.deepgramSaveAndTranscribe(audioBuffer);
      ...
      setTimeout(() => {
        console.log(
          "Timeout reached, stopping recording. State:",
          mediaRecorder.state,
        );
        if (levelInterval) {
          clearInterval(levelInterval);
