...
export class DeepgramSTT extends EventEmitter {
      ...
      this.connection.on(LiveTranscriptionEvents.Transcript, (data: any) => {
        ...
        if (data.channel?.alternatives?.[0]) {
          ...
          if (result.transcript) {
            ...
            const finalIndicator = result.isFinal ? '✓' : '⋯';
            console.log(`${finalIndicator} 🎤 ${result.transcript} ${confidenceColor}(${(result.confidence * 100).toFixed(1)}%)${this.resetColor()}`);
    ...
    if (this.connection) {
      try {
        ...
        if (this.latencyLog.length > 0) {
          const avgLatency = this.latencyLog.reduce((sum, log) => sum + log.latency, 0) / this.latencyLog.length;
          console.log(`📊 Average latency: ${avgLatency.toFixed(2)}ms over ${this.latencyLog.length} transcriptions`);
          this.latencyLog = [];
        }
      } catch (error) {
  ...
  private getConfidenceColor(confidence: number): string {
    if (confidence >= 0.90) return '\x1b[92m'; // GREEN
    if (confidence >= 0.80) return '\x1b[93m'; // YELLOW
    if (confidence >= 0.70) return '\x1b[91m'; // ORANGE (using red as fallback)
    return '\x1b[31m'; // RED
  }
