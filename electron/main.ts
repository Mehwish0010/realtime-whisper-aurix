...
import {
  ...
  getDeepgramInstance,
  transcribeAudioFile,
} from "./deepgram-stt.js";
import {
  createGroqConversationManager,
  getGroqConversationManager,
} from "./groq-chat.js";
import {
...
ipcMain.handle(
  ...
  async (_event, audioPath: string) => {
    try {
      ...
      if (!deepgram) {
        throw new Error(
          "Deepgram not initialized. Please check your API key in .env file.",
        );
      }
...
ipcMain.handle(
  "deepgram-save-and-transcribe",
  async (_event, audioBuffer: ArrayBuffer) => {
    try {
      console.log(
        "Received audio buffer for Deepgram, size:",
      ...
      if (audioBuffer.byteLength < 1000) {
        throw new Error(
          "Audio file too small. Recording may have failed. Please try again.",
        );
      }
...
console.log(
  "AURIX Voice Assistant - Deepgram STT + TTS + Groq Chat Integration Ready",
);
